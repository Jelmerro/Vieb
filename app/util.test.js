...
test("Filesystem helpers should work as expected", () => {
    ...
    strictEqual(
        basePath("/home/test/"), basename("/home/test/"))
    strictEqual(
        isAbsolutePath("/home/test/"), isAbsolute("/home/test/"))
})

test("formatSize helper to show file sizes in human readable form", () => {
    strictEqual(formatSize(45), "45 B")
    strictEqual(formatSize(45345237), "43.24 MB")
    strictEqual(formatSize(92445237293), "86.10 GB")
})
...
test(`Expand path to resolve homedir and downloads`, () => {
    sessionStorage.setItem("settings", JSON.stringify({
        "downloadpath": `~${sep}Downloads${sep}`
    }))
    strictEqual(downloadPath(),
        `${homedir()}${sep}Downloads${sep}`)
    sessionStorage.setItem("settings", JSON.stringify({
        "downloadpath": `Downloads${sep}`
    }))
    strictEqual(downloadPath(), `Downloads${sep}`)
})
